...
public class PandianAccount extends AppCompatActivity implements OnItemClickListener {
    ...
    private void findAllPandianOrder() {
        MyAsyncClient.doPost("findOrderPC.action", new RequestParams(), new MyResponseHandler(PandianAccount.this) {
            @Override
            public void success(JSONObject jsonObject) {
                boolean bool = jsonObject.optString("code").equals("200");
                System.out.println("返回值的length值：" + jsonObject.optJSONArray("data").length());
                int i = 0;
                if (bool) {
                ...
                Toast.makeText(PandianAccount.this, jsonObject.optString("msg"), Toast.LENGTH_SHORT).show();
            }
